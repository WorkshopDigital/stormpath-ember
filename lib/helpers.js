...
module.exports.getUser = function(req, res, next) {
  ...
  if (req.session && req.session.user) {
    ...
    req.app.get('stormpathClient').getAccount(req.session.user.href, options, function(err, account) {
      if (err) {
        req.session.destroy();
        return next();
      }
      ...
      expandAccount(req.app, account, function(err, expandedAccount) {
        ...
        req.session.user = expandedAccount;
  ...
  // authentication.
    ...
    req.app.get('stormpathApplication').authenticateApiRequest({ request: req, locations: ['header'] }, function(err, authResult) {
      if (err) {
        next();
      } else {
